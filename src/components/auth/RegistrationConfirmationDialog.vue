...
// ← props defined first before anything uses it
...
const handleResendEmail = () => {
  ...
  console.log('Resend email requested for:', props.email) // ← now safe
}
</script>
...
<template>
  ...
  >
    <v-card rounded="xl" class="text-center pa-6">
      <!-- Icon Section -->
      <div class="mb-4">
        <v-icon size="80" color="success" class="d-block mx-auto"> mdi-email-check </v-icon>
      </div>
      ...
      <!-- Message -->
      <p class="text-body2 text-grey-darken-1 mb-3">We've sent a confirmation email to:</p>
      ...
      <div class="d-flex gap-3 flex-column flex-sm-row">
        <v-btn variant="tonal" color="primary" class="flex-grow-1" @click="handleResendEmail">
          <v-icon start>mdi-email-resend</v-icon>
          Resend Email
        </v-btn>

        <v-btn variant="flat" color="primary" class="flex-grow-1" @click="handleClose">
